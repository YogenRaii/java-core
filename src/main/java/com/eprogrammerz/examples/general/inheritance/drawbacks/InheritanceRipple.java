...
public class InheritanceRipple {
    ...
    public void testMyHashSet() {
        ...
        assertEquals("Number of attempted adds so far even after removal (expected 3): ", 3, mhs.getAddCount());

        mhs.addAll(Arrays.asList("D", "E", "F"));

        assertEquals("Size of Elements in current set", 5, mhs.size());
        assertEquals("Number of attempted adds so far ",6,  mhs.getAddCount());
    }
}
